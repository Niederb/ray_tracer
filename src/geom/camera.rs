...
pub fn random_in_unit_disk() -> Vec3 {
    loop {
        let v = Vec3::new(rand::random::<f64>(), rand::random::<f64>(), 0.0) * 2.0 - Vec3::new(1.0, 1.0, 0.0);
        if v.length() < 1.0 {
            return v;
        }
    }
...
impl Camera{
    pub fn new(lookfrom:Vec3, lookat:Vec3, vup:Vec3, vfov:f64, aspect:f64, aperture:f64, focus_dist:f64) -> Camera {
        ...
        let lower_left_corner = lookfrom - u * half_width * focus_dist - v * half_width * focus_dist - w * focus_dist; 
        let horizontal = u * 2.0 * half_width * focus_dist;
        let vertical = v * 2.0 * half_height * focus_dist;
        Camera { origin: lookfrom, lower_left_corner, horizontal, vertical, u, v, w, lens_radius }
    }
